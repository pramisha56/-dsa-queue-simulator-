...
void initQueue(Queue* q) {
    q->front = q->rear = NULL;
    q->count = 0;
    q->priority = 0; // Initialize priority to 0
}
...
void renderVehicle(SDL_Renderer *renderer, Vehicle vehicle) {
    ...
    SDL_FRect rect = {vehicle.x, vehicle.y, 10, 30}; // Adjusted vehicle length (10x30)
    SDL_RenderFillRect(renderer, &rect);
}
...
int main(int argc, char *argv[]) {
    ...
    while (running) {
        ...
        if (vehicleQueueA.count > PRIORITY_LANE_THRESHOLD) {
            vehicleQueueA.priority = 1; // Set priority
        } else if (vehicleQueueA.count < 5) {
            vehicleQueueA.priority = 0; // Reset priority
        }
